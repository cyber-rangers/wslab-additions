...
#load LabConfig
    ...
    if ($VMs){
        WriteInfoHighlighted "This script will restore and remove master checkpoints on all VMs listed above. Do you want to do it?"
        if ((read-host "(type Y or N)") -eq "Y"){
            ...
            WriteInfo "Removing master checkpoints..."
            $VMs | Get-VMCheckpoint -Name 'Master CheckPoint' | Remove-VMCheckpoint -confirm:$false
            #finishing    
